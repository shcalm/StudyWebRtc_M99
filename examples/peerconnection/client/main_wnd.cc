...
void MainWnd::StartRemoteRenderer(webrtc::VideoTrackInterface* remote_video) {
  ...
  //render =
  //    webrtc::test::VideoRenderer::CreatePlatformRenderer("direct3d",
  //                                                                 960, 480);
  //remote_video->AddOrUpdateSink(render, rtc::VideoSinkWants());
...
void MainWnd::OnPaint() {
  ...
  //RTC_LOG(LS_INFO) << " MainWnd::OnPaint start";
  ...
  if (ui_ == STREAMING && (remote_renderer /*|| local_renderer*/)) {
    ...
    if (image != NULL) {
      ...
      ::SetStretchBltMode(dc_mem, COLORONCOLOR);
      ...
      if (local_renderer && (rc.right - rc.left) > 200 &&
     ...
     else {        //  // We're still waiting for the video stream to be
    // initialized.
      HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0));
  ...
  // RTC_LOG(LS_INFO) << " MainWnd::OnPaint END";
  ::EndPaint(handle(), &ps);
  RTC_LOG(LS_WARNING) << "onpaint ==================== " << rtc::TimeSince(start);
}
...
void MainWnd::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame) {
  // RTC_LOG(LS_INFO) << " MainWnd::VideoRenderer::OnFrame start";
  int64_t start = rtc::TimeMillis();
  int64_t now_ms = rtc::TimeMillis();
  if ((now_ms - last_ms) > 1000) {
    RTC_LOG(LS_INFO) << " rendered_frame = " << frame_count;
    frame_count = 0;
    last_ms = now_ms;
  }
  frame_count++;
  ...
  if (!this->TryLock()) {
    RTC_LOG(LS_INFO)
        << " MainWnd::VideoRenderer::OnFrame rendering! Dropping frame!";
    return;
  } else {
  ...
  InvalidateRect(wnd_, NULL, TRUE);
  //RTC_LOG(LS_INFO) << " MainWnd::VideoRenderer::OnFrame end";
  RTC_LOG(LS_WARNING) << " OnFrame ==================== " << rtc::TimeSince(start);
}
