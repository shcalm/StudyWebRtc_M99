 ...
 */
 ...
 // onconnect_data_ += message;
  ...
  snprintf(out_buffer, sizeof(out_buffer),
           "PeerConnectionClient::SendToPeer() message  = %s \n",
           message.c_str());
  OutputDebugStringA(out_buffer);
  size_t sent =
      control_socket_->Send(message.c_str(), message.length());
  OutputDebugString(L"PeerConnectionClient::SendToPeer() ok \n ");
...
void PeerConnectionClient::OnConnect(rtc::Socket* socket) {
 ...
 // RTC_DCHECK(sent == onconnect_data_.length());
  std::string myName = "guolb";
  printf("guolb oh my god %s", myName.c_str());
  OutputDebugString(L"guolb oh my god ");
  onconnect_data_.clear();
...
void PeerConnectionClient::OnHangingGetConnect(rtc::Socket* socket) {
  ...
  OutputDebugString(L"guolb oh my god 222 ");
...
void PeerConnectionClient::OnMessageFromPeer(int peer_id,
  ...
  OutputDebugString(L"PeerConnectionClient OnMessageFromPeer ");
...
void PeerConnectionClient::OnRead(rtc::Socket* socket) {
  ...
  OutputDebugString(L"guolb oh my god OnRead \n ");
  char buffer[0xffff];
  ...
  do {
   ...
   // if (bytes <= 0) 
      //  break;
    char buffer1[0xffff];
    snprintf(buffer1, sizeof(buffer1), "data is %s,bytes = %d\n", buffer, bytes);    
    OutputDebugStringA(buffer1);
    if (bytes > 0) {
      recv_data.append(buffer, bytes);
    }
    if (recv_data.size() > 4000) {
     break;
    }
  } while (true);
  char out_buffer[0xffff];
  snprintf(out_buffer, sizeof(out_buffer), "%s,size = %lld \n", recv_data.c_str(),recv_data.size());
  OutputDebugStringA(out_buffer);
  OnMessageFromPeer(0xff, recv_data);
  #if 0
